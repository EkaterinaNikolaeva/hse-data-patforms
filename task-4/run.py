...
# тут нужны несколько трансформаций данных (например, агрегаций или преобразований типов)
writer=DBWriter(connection=hive, target="test.history",options=Hive.WriteOptions(partitionBy=["date"]))
writer.run(df)
spark.stop()
