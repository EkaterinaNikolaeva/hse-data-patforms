...
Эта команда запустит playbook `load_data.yml` который:
- Скачает CSV файл с данными о товарных знаках с сайта Роспатента на jump-узел в `/home/team/downloads/`
- Скопирует файл на NameNode и загрузит в HDFS
- Создаст базу данных `trademarks_db` в Hive
- Создаст партиционированную таблицу `trademarks` с партицией по году из поля `registration_date`
- Загрузит данные, автоматически распределяя их по партициям по годам
- Покажет информацию о созданных партициях и количество записей в каждой
...
**Для подключение используется созданный тунель **
```
...
```sql
-- Просмотр партиций
SHOW PARTITIONS trademarks_db.trademarks;

-- Статистика по годам
SELECT year, COUNT(*) as count 
FROM trademarks_db.trademarks 
GROUP BY year 
ORDER BY year;

-- Выборка данных из конкретной партиции
SELECT * FROM trademarks_db.trademarks 
WHERE year = 2020 
LIMIT 100;
```
...
Playbook `load_data.yml` выполняет загрузку данных о товарных знаках в Hive с партиционированием по году:

#### Этап 1: Загрузка данных на jump-узел
- **Скачивание CSV файла** - загрузка файла с данными о товарных знаках с сайта Роспатента
- **Сохранение в директорию downloads** - файл сохраняется в `/home/team/downloads/` на jump-узле

#### Этап 2: Копирование и загрузка в HDFS
- **Копирование файла на NameNode** - перенос файла с jump-узла на NameNode
- **Предобработка CSV файла** - нормализация CSV файла с помощью Python для корректного парсинга:
  - Удаление BOM символа (UTF-8 BOM) из начала файла
  - Правильная обработка значений в кавычках с запятыми внутри
  - Нормализация экранирования кавычек согласно стандарту RFC 4180
  - Перезапись файла в формате, совместимом с OpenCSVSerde
- **Создание директорий в HDFS** - создание staging и финальной директорий для данных
- **Очистка старых данных** - удаление старых файлов из HDFS директорий перед загрузкой новых данных
- **Загрузка в HDFS** - загрузка предобработанного CSV файла в staging директорию HDFS

#### Этап 3: Создание структуры в Hive
- **Удаление существующих таблиц** - удаление старых таблиц перед созданием новых для обеспечения чистой загрузки
- **Создание базы данных** - создание базы данных `trademarks_db`
- **Создание staging таблицы** - создание временной таблицы для первоначальной загрузки данных с использованием OpenCSVSerde для корректного парсинга CSV (все поля STRING)
- **Создание партиционированной таблицы** - создание таблицы `trademarks` с партицией по году (INT) и правильными типами данных:
  - **Первичный ключ**: `registration_number` (BIGINT) - уникальный идентификатор записи (валидация выполняется в WHERE условии при загрузке)
  - Обязательные поля: `registration_number` и `registration_date` - валидация выполняется в WHERE условии при загрузке
  - Числовые поля: `BIGINT` для номеров регистрации, `INT` для дат (формат YYYYMMDD) и номеров заявок
  - Логические поля: `BOOLEAN` для полей `collective`, `threedimensional`, `holographic`, `sound`, `olfactory`, `color`, `light`, `changing`, `positional`, `actual`
  - Текстовые поля: `STRING` для всех остальных полей (адреса, названия, описания и т.д.)

#### Этап 4: Загрузка данных с партиционированием
- **Преобразование типов данных** - преобразование данных из staging таблицы (все STRING) в правильные типы:
  - `CAST` для числовых полей (BIGINT, INT) с обработкой пустых значений через `NULLIF`
  - `CAST` для логических полей (BOOLEAN) с преобразованием строк 'true'/'false' в boolean значения
  - Обработка пустых строк - пустые значения преобразуются в NULL для числовых и логических полей
- **Извлечение года из даты** - извлечение года из поля `registration_date` (формат YYYYMMDD)
- **Динамическое создание партиций** - автоматическое создание партиций по годам
- **Распределение данных** - загрузка данных в соответствующие партиции по годам

#### Этап 5: Отображение результатов
- **Показ партиций** - вывод списка всех созданных партиций
- **Статистика по партициям** - вывод количества записей в каждой партиции

#### Этап 6: Очистка
- **Удаление staging таблицы** - удаление временной таблицы после загрузки
- **Удаление staging директории** - очистка временных файлов из HDFS
- **Удаление локальных файлов** - удаление временных файлов с серверов
